---
author: douglaslMS
ms.service: sql-database
ms.topic: include
ms.date: 11/25/2018
ms.author: douglasl
ms.openlocfilehash: 5f92b0a237bb25684f14edbd7cdd5d9f92ff9cb3
ms.sourcegitcommit: 829d951d5c90442a38012daaf77e86046018e5b9
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 10/09/2020
ms.locfileid: "87103010"
---
Azure Database Migration Service es un servicio totalmente administrado diseñado para permitir migraciones completas desde varios orígenes de base de datos hasta las plataformas de datos de Azure con un tiempo de inactividad mínimo.

| **-resource** | **LIMIT** | **Comentarios** |
| --- | --- | --- |
| Número máximo de servicios por suscripción por región |10 | Para solicitar un aumento de este límite, póngase en contacto con el soporte técnico. |
