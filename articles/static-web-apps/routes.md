...
ms.locfileid: "83595134"
---
# <a name="routes-in-azure-static-web-apps-preview"></a>Rutas en la versión preliminar de Azure Static Web Apps

El enrutamiento en Azure Static Web Apps define las reglas de enrutamiento del back-end y el comportamiento de autorización para las API y el contenido estático. Se definen como una matriz de reglas en el archivo _routes.json_.
...
| `serve`        | No       | N/D          | Define el archivo o la ruta de acceso que devuelve la solicitud. La ruta de acceso y el nombre del archivo pueden ser distintos de la ruta de acceso solicitada. Si se define un valor `serve`, se usa la ruta de acceso solicitada. |
...
| _/admin/reports_ | Los usuarios autenticados en el rol _Administradores_ reciban el archivo _/admin/reports/index.html_. Los usuarios autenticados que no tienen el rol _Administradores_ reciben un error 401<sup>1</sup>. Los usuarios sin autenticar se redirigen a _/login_. |
...
| _/customers/contoso_ | Los usuarios autenticados que pertenezcan a los roles _Administradores_ o _customers\_contoso_ reciban el archivo _/customers/contoso/index.html_<sup>1</sup>. Los usuarios autenticados que no tienen los roles _Administradores_ o _customers\_contoso_ reciben un error 401. Los usuarios sin autenticar se redirigen a _/login_. |
...
<sup>1</sup> Puede proporcionar una página de error personalizada al definir una regla `Unauthorized_MissingRoles` en la matriz `platformErrorOverrides`.
