...
En la versión preliminar púbica, Synapse Link es compatible con SQL (Core) API de Azure Cosmos DB y Azure Cosmos DB API para MongoDB. La compatibilidad con Cassandra API está actualmente en versión preliminar controlada. Para solicitar acceso a la versión preliminar validada, póngase en contacto con el [equipo de Azure Cosmos DB](mailto:cosmosdbsynapselink@microsoft.com).
...
|Synapse SQL sin servidor    | Lectura, vista |
