...
## <a name="issues-with-creating-or-modifying-a-gallery"></a>Problemas con la creación o la modificación de una galería ##
...
**Causa** : El nombre especificado para la galería no cumple los requisitos de nomenclatura.  
**Solución alternativa** : Elija un nombre que cumpla las condiciones siguientes: 1) Límite de 80 caracteres, 2) contiene solo letras, números, guiones bajos y puntos en inglés, 3) comienza y termina con letras o números en inglés.
...
**Solución alternativa** : Elija un nombre para la galería que cumpla las condiciones siguientes: 1) Límite de 80 caracteres, 2) contiene solo letras, números, guiones bajos y puntos en inglés, 3) comienza y termina con letras o números en inglés.
...
**Solución alternativa** : Elimine todas las definiciones de imagen que contiene la galería y, luego, proceda a la eliminación de la galería. Si la definición de imagen contiene versiones de imagen, se deben eliminar estas últimas antes de eliminar la primera.
...
**Causa** : Ya tiene una galería en el grupo de recursos con el mismo nombre y ha intentado crear otra galería con el mismo nombre, pero en otra región.  
**Solución alternativa** : Use otra galería y otro grupo de recursos.

## <a name="issues-with-creating-or-modifying-image-definitions"></a>Problemas con la creación o la modificación de definiciones de imagen ##
...
**Causa** : Ya tiene una definición de imagen existente en la misma galería y grupo de recursos con el mismo nombre y ha intentado crear otra definición de imagen con el mismo nombre y en la misma galería, pero en otra región.  
...
**Causa** : El valor de <imageDefinitionName\> dado termina con un punto o un guion bajo.  
**Solución alternativa** : Elija un nombre para la definición de imagen que cumpla las condiciones siguientes: 1) Límite de 80 caracteres, 2) contiene solo letras, números, guiones, guiones bajos y puntos del alfabeto inglés, 3) comienza y termina con letras o números en inglés.
...
**Causa** : El valor de editor, oferta o SKU dado no cumplen los requisitos de nomenclatura.  
**Solución alternativa** : Elija un valor que cumpla las condiciones siguientes: 1) Límite de 128 caracteres para el editor o de 64 caracteres para la oferta y la SKU, 2) contiene solo letras, números, guiones, guiones bajos y puntos del alfabeto inglés y 3) no termina con un punto.
...
*La ubicación proporcionada <region\> no está disponible para el tipo de recurso "Microsoft.Compute/galeries". La lista de regiones disponibles para el tipo de recurso es...*  
**Causa** : El valor de <region\> es incorrecto o se requiere una solicitud de acceso.  
**Solución alternativa** : Compruebe que el nombre de la región esté escrito correctamente. Puede ejecutar este comando para ver a qué regiones tiene acceso. Si la región no aparece en la lista, envíe [una solicitud de acceso](/troubleshoot/azure/general/region-access-request-process).
...
**Solución alternativa** : En una galería determinada, todas las definiciones de imagen deben tener una combinación única de editor, oferta y SKU. Elija una combinación única o elija una nueva galería y vuelva a crear la definición de imagen.
...
**Causa** : Ha elegido un tipo ZRS estándar en una región que todavía no lo admite.  
**Solución alternativa** : Cambie el tipo de cuenta de almacenamiento a "Premium\_LRS" o "Standard\_LRS". Consulte la documentación para obtener la [lista de regiones](/azure/storage/common/storage-redundancy#zone-redundant-storage) más reciente con la versión preliminar de ZRS habilitada.

## <a name="issues-with-creating-or-updating-image-versions"></a>Problemas con la creación o la actualización de versiones de imagen ##
...
**Solución alternativa** : Asegúrese de que el identificador de origen sea el mismo que el identificador de origen ya existente o cambie el número de versión de la versión de imagen.
...
**Causa** : Al crear una versión de imagen mediante una lista de discos o instantáneas de disco, dos o más discos o instantáneas de disco tienen los mismos números LUN.  
**Solución alternativa** : Quite o cambie los números LUN duplicados.
...
**Causa** : El formato de <resourceID\> no es correcto.  
**Solución alternativa** : Compruebe que el valor de resourceID sea correcto.
...
**Causa** : El origen de "<resourceID\>" contiene un disco de SO efímero. Shared Image Gallery no admite actualmente discos de SO efímeros.  
...
**Causa** : El valor de "<diskID\> del disco es un disco UltraSSD. Shared Image Gallery no admite actualmente discos SSD Ultra.  
...
**Solución alternativa** : Pruebe a crear la versión de imagen más tarde.
...
**Causa** : Shared Image Gallery no admite actualmente la creación de versiones de imagen en otra suscripción desde otra imagen de origen si esta está cifrada.  
...
**Causa** : El formato válido para una versión de imagen es de 3 enteros separados por un punto. El nombre de la versión de imagen no cumplía el formato válido.  
...
**Causa** : Puede que el valor de resourceID del origen no sea correcto.  
**Solución alternativa** : Asegúrese de que el valor de resourceID del origen sea correcto.
...
**Solución alternativa** : Si usa cifrado en el disco del sistema operativo, use cifrado en todas las regiones.
...
**Solución alternativa** : Si usa cifrado en un disco de datos, use cifrado en todas las regiones.

*Se especificó un LUN no válido [<number\>] en encryption.dataDiskImages. El LUN debe tener uno de los siguientes valores ["0,9"].*  
**Causa** : El número LUN especificado para el cifrado no coincide con ninguno de los números de LUN de los discos asociados a la máquina virtual.  
**Solución alternativa** : Cambie el número LUN del cifrado por el número de LUN de un disco de datos presente en la máquina virtual.

*Se especificaron LUN duplicados "<number\>" en la región de destino "<region\>" encryption.dataDiskImages.*  
**Causa** : La configuración de cifrado usada en <region\> ha especificado un número LUN al menos dos veces.  
**Solución alternativa** : Cambie el número LUN de <region\> para asegurarse de que todos los números LUN sean únicos en <region\>.

*OSDiskImage y DataDiskImage no pueden apuntar al mismo blob < sourceID\>* .  
**Causa** : El origen del disco del sistema operativo y al menos un disco de datos no son únicos.  
...
**Causa** : Una llamada de actualización a la versión de imagen contiene un nuevo disco de datos o tiene un nuevo número LUN para un disco.  
**Solución alternativa** : Use los números LUN y los discos de datos de la versión de imagen existente.

*El conjunto de cifrado de disco "<diskEncryptionSetID\>" debe estar en la misma suscripción "<subscriptionID\>" que el recurso de la galería.*  
**Causa** : Shared Image Gallery no admite actualmente el uso de un conjunto de cifrado de disco en otra suscripción.  
**Solución alternativa** : Cree la versión de imagen y conjunto de cifrado de disco en la misma suscripción.

## <a name="issues-creating-or-updating-a-vm-or-scale-sets-from-image-version"></a>Problemas al crear o actualizar una máquina virtual o conjuntos de escalado a partir de la versión de imagen ##
...
**Causa** : La máquina virtual o el conjunto de escalado se crearon con una imagen SIG en otro inquilino. Ha intentado realizar un cambio en la máquina virtual o el conjunto de escalado, pero no tiene acceso a la suscripción que posee la imagen.  
...
**Solución alternativa** : Compruebe que el valor de **OSType** de la versión de la imagen a partir de la cual intenta crear la VM o el conjunto de escalado de máquinas virtuales tiene el mismo valor de **OSType** que el origen que usó para crear la versión de la imagen. 

## <a name="issues-creating-a-disk-from-an-image-version"></a>Problemas al crear un disco a partir de una versión de imagen ##
