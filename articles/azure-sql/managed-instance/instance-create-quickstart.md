---
title: 'Inicio rápido: Creación de una instancia administrada de Azure SQL (Azure Portal)'
description: En este inicio rápido, creará una instancia administrada de Azure SQL, un entorno de red y una máquina virtual cliente para el acceso mediante Azure Portal.
...
Para crear una instancia administrada de Azure SQL, siga estos pasos: 
...
1. Seleccione **+ Agregar** para abrir la página **Select SQL deployment option** (Seleccionar la opción de implementación de SQL). Para ver información adicional acerca de una instancia administrada de Azure SQL, seleccione **Mostrar detalles** en el icono **Instancia administrada de SQL**.
1. Seleccione **Crear**.

   ![Creación de una instancia administrada de SQL](./media/instance-create-quickstart/create-managed-instance.png)
...
- Rellene la información obligatoria necesaria en la pestaña **Aspectos básicos**. Es un conjunto mínimo de información necesaria para aprovisionar una instancia administrada de SQL.
   ...
   ![Pestaña "Aspectos básicos" para crear una instancia administrada de SQL](./media/instance-create-quickstart/mi-create-tab-basics.png)
   ...
   | **Región** |La región en la que desea crear la instancia administrada de SQL.|Para obtener información sobre las regiones, consulte [Regiones de Azure](https://azure.microsoft.com/regions/).|
...
- Rellene la información opcional de la pestaña **Redes**. Si omite esta información, el portal aplicará la configuración predeterminada.
   ...
   ![Pestaña "Redes" para crear una instancia administrada de SQL](./media/instance-create-quickstart/mi-create-tab-networking.png)

   Use la tabla siguiente como referencia para obtener la información necesaria en esta pestaña.

   | Configuración| Valor sugerido | Descripción |
   | ------ | --------------- | ----------- |
   | **Red virtual** | Seleccione **Crear una nueva red virtual** o una subred y red virtual válidas.| Si alguna red o subred no está disponible, se debe [modificar para que cumpla con los requisitos de red](vnet-existing-add-subnet.md) antes de seleccionarla como destino de la nueva instancia administrada de SQL. Para más información acerca de los requisitos necesarios para configurar el entorno de red de una instancia administrada de SQL, consulte el artículo en el que se indica cómo [configurar una red virtual para una instancia administrada de Azure SQL](connectivity-architecture-overview.md). |
   ...
   | **Punto de conexión público**  | Seleccione **Habilitar**. | Para que se pueda acceder a una instancia administrada de SQL mediante el punto de conexión de datos público, es preciso habilitar esta opción. | 
   | **Permitir acceso desde** (si la opción **Punto de conexión público**  está habilitada) | Seleccione una de las opciones.   |La experiencia del portal permite configurar un grupo de seguridad con un punto de conexión público. </br> </br> En función del escenario, seleccione una de las siguientes opciones: </br> <ul> <li>**Servicios de Azure**: esta opción es aconsejable cuando se establece conexión desde Power BI u otro servicio multiinquilino. </li> <li> **Internet**: se usa para realizar pruebas cuando desea poner en marcha rápidamente una instancia administrada de SQL. No es aconsejable para entornos de producción. </li> <li> **Sin acceso**: esta opción crea una regla de seguridad **Denegar**. Modifique esta regla para que se pueda acceder a la instancia administrada de SQL mediante un punto de conexión público. </li> </ul> </br> Para más información acerca de la seguridad de los puntos de conexión públicos, consulte el artículo sobre el [uso de Instancia administrada de Azure SQL de forma segura con puntos de conexión públicos](public-endpoint-overview.md).|

- Seleccione **Revisar y crear** para revisar las opciones elegidas antes de crear una instancia administrada de SQL. O bien, configure más opciones personalizadas, para lo que debe seleccionar **Siguiente: Configuración adicional**.
...
- Rellene la información opcional de la pestaña **Configuración adicional**. Si omite esta información, el portal aplicará la configuración predeterminada.
   ...
   ![Pestaña "Configuración adicional" para crear una instancia administrada de SQL](./media/instance-create-quickstart/mi-create-tab-additional-settings.png)

   Use la tabla siguiente como referencia para obtener la información necesaria en esta pestaña.

   | Configuración| Valor sugerido | Descripción |
   | ------ | --------------- | ----------- |
   | **Intercalación** | Elija la intercalación que desea usar para la instancia administrada de SQL. Si migra bases de datos desde SQL Server, compruebe la intercalación de origen mediante `SELECT SERVERPROPERTY(N'Collation')` y use ese valor.| Para obtener información acerca de las intercalaciones, consulte [Configurar o cambiar la intercalación del servidor](https://docs.microsoft.com/sql/relational-databases/collations/set-or-change-the-server-collation).|   
   | **Zona horaria** | Seleccione la zona horaria que la instancia administrada de SQL va a emplear.|Para más información, consulte las [zonas horarias](timezones-overview.md).|
   | **Use as failover secondary** (Usar como secundaria de conmutación por error) | Seleccione **Sí**. | Habilite esta opción para usar la instancia administrada de SQL como secundaria de un grupo de conmutación por error.|
   | **Instancia administrada de SQL principal** (si en **Usar como secundario de conmutación por error** se ha seleccionado **Sí**) | Elija la instancia administrada de SQL principal existente que se unirá a la misma zona DNS con la instancia administrada de SQL que está creando. | Este paso habilitará la configuración posterior a la creación del grupo de conmutación por error. Para más información, consulte el [Tutorial: Agregue una instancia administrada de SQL Database a un grupo de conmutación por error](failover-group-add-instance-tutorial.md).|

## <a name="review--create"></a>Revisar y crear

1. Para revisar las opciones elegidas antes de crear la instancia administrada de SQL, seleccione la pestaña **Revisar y crear**.

   ![Pestaña para examinar y crear una instancia administrada de SQL](./media/instance-create-quickstart/mi-create-tab-review-create.png)

1. Seleccione **Crear** para iniciar el aprovisionamiento de la instancia administrada de SQL.

> [!IMPORTANT]
> La implementación de una instancia administrada de SQL es una operación de ejecución prolongada. La implementación de la primera instancia en la subred habitualmente tarda mucho más que la implementación en una subred en la que ya existen instancias administradas de SQL. Para conocer los tiempos de aprovisionamiento medios, consulte el apartado sobre las [operaciones de administración de Instancia administrada de SQL](sql-managed-instance-paas-overview.md#management-operations).
...
> 1. En Azure Portal, abra el grupo de recursos (en la pestaña **Aspectos básicos**) en el que va a implementar una instancia administrada de SQL.
> 2. Seleccione **Implementaciones**.
> 3. Seleccione la operación de implementación de la instancia administrada de SQL en curso.

> [!IMPORTANT]
> Para poder obtener el estado de creación de una instancia administrada de SQL, es preciso tener **permisos de lectura** en el grupo de recursos. Si no tiene este permiso o lo revoca mientras la instancia administrada de SQL se encuentra en proceso de creación, es posible que la instancia administrada de SQL no se vea en la lista de implementaciones del grupo de recursos.
>

## <a name="view-resources-created"></a>Visualización de los recursos creados

Tras una implementación correcta de Instancia administrada de SQL, para ver los recursos creados:

1. Abra el grupo de recursos de la instancia administrada de SQL. 
...
> [!IMPORTANT]
> Si ha configurado un punto de conexión público para la instancia administrada de SQL, es preciso que abra los puertos necesarios para que haya tráfico de red que permita establecer conexiones a Instancia administrada de SQL desde la red pública de Internet; consulte el artículo en que se indica cómo [configurar un punto de conexión público para una instancia administrada de SQL](public-endpoint-configure.md#allow-public-endpoint-traffic-on-the-network-security-group) para más información.
>
...
Para conectarse a una instancia administrada de SQL, siga estos pasos para recuperar el nombre de host y el nombre de dominio completo (FQDN):

1. Vuelva al grupo de recursos y seleccione una instancia administrada de SQL.

   ![Instancia administrada de SQL en el grupo de recursos](./media/instance-create-quickstart/managed-instance.png)

2. En la pestaña **Información general**, busque la propiedad **Host**. Copie el nombre de host de la instancia administrada de SQL para usarlo en el siguiente inicio rápido.
...
Para aprender a conectarse a una instancia administrada de SQL:
- Para obtener información general acerca de las opciones de conexión de las aplicaciones, consulte el artículo sobre la [conexión de aplicaciones a una instancia administrada de SQL](connect-application-instance.md).
- Para ver un inicio rápido que muestre cómo conectarse a una instancia administrada de SQL desde una máquina virtual de Azure, consulte el documento sobre la [configuración de la conexión a una máquina virtual de Azure](connect-vm-instance-configure.md).
- Para ver un inicio rápido que muestre cómo conectarse a una instancia administrada de SQL desde un equipo cliente local mediante una conexión de punto a sitio, consulte el artículo acerca de la [configuración de una conexión de punto a sitio](point-to-site-p2s-configure.md).

Para restaurar una base de datos de SQL Server desde un entorno local a una instancia administrada de SQL: 
