...
La marca de tiempo de los nombres de blobs en la carpeta `PT=Time` corresponde a la hora de llegada de los datos a TSI (no a la marca de tiempo de los eventos).

Los datos de la carpeta `PT=TsId` se optimizan para la consulta a lo largo del tiempo y no están estáticos. Durante la creación de particiones, es posible que los mismos eventos estén presentes en varios blobs. Además, la nomenclatura de los blobs podría cambiar en el futuro.
