---
title: Consulta de datos de Azure Cosmos DB mediante SQL sin servidor en Azure Synapse Link (versión preliminar)
...
> [!NOTE]
> SQL sin servidor no admite la consulta del almacén transaccional de Azure Cosmos DB.
...
Para seguir con este artículo, en el que se muestra cómo consultar datos de Cosmos DB con SQL sin servidor, asegúrese de que crea los recursos siguientes:
...
```
En el ejemplo anterior, se indica a SQL sin servidor que se conecte a la base de datos `covid` en la cuenta de Azure Cosmos DB `MyCosmosDbAccount` autenticada con la clave de Azure Cosmos DB (en el ejemplo es ficticia). Después, se accede al almacén analítico del contenedor `EcdcCases` en la región `West US 2`. Como no se proyectan propiedades específicas, la función `OPENROWSET` devolverá todas las propiedades de los elementos de Azure Cosmos DB.
...
Obtenga más información sobre cómo analizar [tipos de datos complejos en Synapse Link](../how-to-analyze-complex-schema.md) y [estructuras anidadas en SQL sin servidor](query-parquet-nested-types.md).
...
En algunos casos, puede que tenga que "combinar" las propiedades del elemento superior (metadata) con todos los elementos de la matriz (authors). SQL sin servidor permite acoplar estructuras anidadas si se aplica la función `OPENJSON` en la matriz anidada:
...
- SQL sin servidor de Synapse actualmente no admite el [esquema con fidelidad total de Azure Cosmos DB](../../cosmos-db/analytical-store-introduction.md#schema-representation). Use SQL sin servidor de Synapse solo para acceder al esquema bien definido de Cosmos DB.
