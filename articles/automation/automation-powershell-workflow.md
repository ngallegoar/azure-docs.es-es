---
title: Aprendizaje del flujo de trabajo de PowerShell para Azure Automation
...
> [!NOTE]
> No se recomienda ejecutar runbooks secundarios en paralelo, ya que se ha demostrado que no tiene unos resultados confiables. A veces el resultado del runbook secundario no se muestra, y la configuración de un runbook secundario puede afectar a los demás runbooks secundarios paralelos. Variables como $VerbosePreference, $WarningPreference y otras no se pueden propagar a los runbooks secundarios. Y si el runbook secundario cambia estos valores, es posible que no se restauren correctamente después de la invocación.
