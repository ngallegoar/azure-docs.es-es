...
ms.locfileid: "80238671"
---
# <a name="accelerated-database-recovery"></a>Recuperación acelerada de bases de datos

**Recuperación de base de datos acelerada (ADR)** es una característica del motor de base de datos SQL que mejora considerablemente la disponibilidad de la base de datos, en especial en presencia de transacciones de larga duración, al rediseñar el proceso de recuperación del motor de base de datos SQL. ADR está disponible actualmente para la instancia administrada y el grupo elástico únicos de Azure SQL Database instancia administrada y un grupo elástico único, y para las bases de datos de Azure SQL Data Warehouse (actualmente en versión preliminar). Las principales ventajas de ADR son:
...
En función de este diseño, el tiempo que tarda el motor de base de datos SQL en recuperarse de un reinicio inesperado es (aproximadamente) proporcional al tamaño de la transacción activa más larga del sistema en el momento del bloqueo. La recuperación requiere la reversión de todas las transacciones incompletas. El período de tiempo necesario es proporcional al trabajo que ha realizado la transacción y el tiempo que ha estado activa. Por lo tanto, el proceso de recuperación de SQL Server puede tardar mucho tiempo en la presencia de transacciones de larga duración (como operaciones de inserción masiva grandes u operaciones de generación de índice en una tabla grande).
...
![Proceso de recuperación de ADR](./media/sql-database-accelerated-database-recovery/adr-recovery-process.png)
...
- Las cargas de trabajo que han experimentado largos períodos de falta de disponibilidad de la base de datos debido a la recuperación de larga duración de SQL Server (por ejemplo, un reinicio inesperado de SQL Server o la reversión manual de transacciones).
