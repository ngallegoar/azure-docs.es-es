---
title: Ejemplos de una directiva de Azure Attestation
description: Ejemplos de la directiva de Azure Attestation.
services: attestation
author: msmbaldwin
ms.service: attestation
ms.topic: overview
ms.date: 08/31/2020
ms.author: mbaldwin
ms.openlocfilehash: 537f02b26c4ea3be59097a120482f98109f8b16a
ms.sourcegitcommit: d68c72e120bdd610bb6304dad503d3ea89a1f0f7
ms.translationtype: HT
ms.contentlocale: es-ES
ms.lasthandoff: 09/01/2020
ms.locfileid: "89237161"
---
# <a name="examples-of-an-attestation-policy"></a>Ejemplos de una directiva de atestación

La directiva de atestación se usa para procesar la evidencia de atestación y determinar si Azure Attestation emitirá un token de atestación. La generación de tokens de atestación se puede controlar con directivas personalizadas. A continuación se muestran algunos ejemplos de una directiva de atestación.

## <a name="default-policy-for-an-sgx-enclave-with-policyformattext"></a>Directiva predeterminada para un enclave de SGX con PolicyFormat=Text

```
Version= 1.0;
authorizationrules
{
    c:[type==”$is-debuggable”] => permit();
};
issuancerules
{
    c:[type==”$is-debuggable”] => issue(type=”is-debuggable”, value=c.value);
    c:[type==”$sgx-mrsigner”] => issue(type=”sgx-mrsigner”, value=c.value);
    c:[type==”$sgx-mrenclave”] => issue(type=”sgx-mrenclave”, value=c.value);
    c:[type==”$product-id”] => issue(type=”product-id”, value=c.value);
    c:[type==”$svn”] => issue(type=”svn”, value=c.value);
    c:[type==”$tee”] => issue(type=”tee”, value=c.value);
};
```

## <a name="default-policy-for-vbs-enclave"></a>Directiva predeterminada para un enclave de VBS

No hay ninguna directiva predeterminada para el enclave de VBS


## <a name="unsigned-policy-for-an-sgx-enclave-with-policyformatjwt"></a>Directiva sin firmar para un enclave de SGX con PolicyFormat=JWT

```
eyJhbGciOiJub25lIn0..
```

## <a name="signed-policy-for-an-sgx-enclave-with-policyformatjwt"></a>Directiva firmada para un enclave de SGX con PolicyFormat=JWT

```
.-IGlzc3VlKHR5cGU9XCJzZ3gtbXJzaWduZXJcIiwgdmFsdWU9Yy52YWx1ZSk7YzpbdHlwZT09XCIkc2d4LW1yZW5jbGF2ZVwiXSA9PiBpc3N1ZSh0eXBlPVwic2d4LW1yZW5jbGF2ZVwiLCB2YWx1ZT1jLnZhbHVlKTtjOlt0eXBlPT1cIiRwcm9kdWN0LWlkXCJdID0-.Rm_KOcxPzo_6AytCyoWrgRa50efT_87gR9_Yewxeuesji99drOepXp50qabnPc0oCyy5ArcPSETeYCaYQrrNVhCTEW-3U_WJxXKrHuQIa9IcE06muFrA4X4esjCitbgrTujKzf83puB5u8vUG0KI_y9FAKGIjrZzDRXAG0126DOCjMltUxyRpNpXq5Ex6SQI5q-CQctoO9kYkMrYoUTqAxiAJ8mpNvC4Ufe4TjF8t6SpSI2zXXta_gY_decJ8AkyDx4YkzbHn5QzjaXaXoHMgbj_76D_RnGhANC0u5rBHvqScFMTO50QnPW94frkn-9PBtWuQ0g7Q1TeERkoMknhBA       
```

</br>

## <a name="next-steps"></a>Pasos siguientes

- [Creación y firma de una directiva de atestación](author-sign-policy.md)
- [Configuración de Azure Attestation con PowerShell](quickstart-powershell.md)
