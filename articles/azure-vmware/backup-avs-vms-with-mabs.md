...
En este artículo, se describen los procedimientos para hacer una copia de seguridad de máquinas virtuales (VM) de VMware que se ejecutan en Azure VMware Solution mediante Azure Backup Server. Antes de empezar, asegúrese de consultar detenidamente [Configuración de Microsoft Azure Backup Server para Azure VMware Solution](set-up-mabs-for-avs.md).
...
## <a name="add-the-provisioning-ip-address-for-azure-vmware-solution-esxi-hosts-on-azure-backup-server"></a>Incorporación de la dirección IP de aprovisionamiento para los hosts ESXi de Azure VMware Solution en Azure Backup Server

Durante la versión preliminar, Azure VMware Solution no resuelve el host ESX desde la máquina virtual implementada en la red virtual. Tendrá que realizar pasos adicionales para agregar la entrada de archivo de host en la máquina virtual de Azure Backup Server.

### <a name="identify-the-ip-address-for-esxi-hosts"></a>Identificación de la dirección IP de los hosts ESXi

1. Abra el explorador e inicie sesión en las direcciones URL de vCenter. 

   > [!TIP]
   > Puede encontrar las direcciones URL en [Conexión a la instancia local de vCenter de la nube privada](tutorial-access-private-cloud.md#connect-to-the-local-vcenter-of-your-private-cloud).

1. En el cliente de vSphere, seleccione el clúster para el que va a habilitar la copia de seguridad.

   :::image type="content" source="media/avs-backup/vsphere-client-select-host.png" alt-text="Cliente de vSphere: seleccionar host":::

1. Seleccione **Configurar** > **Redes** > **Adaptadores de VMKernel**. En la lista de dispositivos, identifique el adaptador de red que tiene habilitado el rol **Aprovisionamiento**. Anote la **dirección IP** y el nombre de host de ESXi.

   :::image type="content" source="media/avs-backup/vmkernel-adapters-provisioning-enabled.png" alt-text="Adaptadores de VMKernel: aprovisionamiento de dispositivos habilitados":::

1. Repita el paso anterior para cada host ESXi en cada clúster para el que vaya a habilitar la copia de seguridad.

### <a name="update-the-host-file-on-azure-backup-server"></a>Actualización del archivo de host en Azure Backup Server

1. Abra el Bloc de notas como administrador.

1. Seleccione **Archivo** > **Abrir** y busque c:\Windows\System32\Drivers\etc\hosts.

1. Agregue la entrada para cada host ESXi junto con la dirección IP que identificó en la sección anterior.
...
1. En el cuadro de diálogo **Administrar credenciales**, seleccione **Agregar**.
   ...
   ![Cuadro de diálogo Administrar credenciales de Azure Backup Server](../backup/media/backup-azure-backup-server-vmware/mabs-manage-credentials-dialog.png)
...
1. Haga clic en **Agregar** para agregar la nueva credencial.
   ...
   ![Cuadro de diálogo Administrar credenciales de Azure Backup Server](../backup/media/backup-azure-backup-server-vmware/new-list-of-mabs-creds.png)
...
1. En la página **Resumen**, revise la configuración y seleccione **Finalizar**.
   ...
   Debería ver el servidor vCenter que aparece en **Servidor de producción** con el tipo **Servidor de VMware** y **Estado del agente** como **Aceptar**. Si ve **Estado del agente** como **Desconocido**, seleccione **Actualizar**.
...
1. Especifique cuánto tiempo quiere mantener la copia de seguridad de los datos en el disco.
   ...
   :::image type="content" source="media/avs-backup/new-protection-group-specify-short-term-goals.png" alt-text="Especificar los objetivos a corto plazo para la protección en disco":::
...
1. En **Revisar asignación de Disk Storage**, revise el espacio en disco para las copias de seguridad de máquinas virtuales.
   ...
   - **Espacio en disco:** espacio en disco recomendado para el grupo de protección. Si quiere modificar esta configuración, asigne un espacio total que sea ligeramente mayor que la cantidad que calcula que va a crecer cada origen de datos.
   ...
   :::image type="content" source="media/avs-backup/review-disk-allocation.png" alt-text="Revisar el espacio en disco asignado en el bloque de almacenamiento":::
...
1. En la página **Especificar la programación de copia de seguridad en línea**, indique con qué frecuencia quiere realizar la copia de seguridad de los datos del espacio de almacenamiento local en Azure y, después, seleccione **Siguiente**. 
...
1. En la página **Especificar la directiva de retención en línea**, indique el tiempo de conservación de los puntos de recuperación que se crean de las copias de seguridad diarias, semanales, mensuales y anuales en Azure y, después, seleccione **Siguiente**.
...
- En la pestaña **Alertas** del panel **Supervisión**, puede supervisar los errores, las advertencias y la información general de un grupo de protección, de un equipo protegido específico o de la gravedad del mensaje. Puede ver las alertas activas e inactivas y configurar las notificaciones por correo electrónico.
- En la pestaña **Trabajos** del panel **Supervisión**, puede ver los trabajos iniciados por Azure Backup Server para un grupo de protección o un origen de datos protegido específico. Puede seguir el progreso del trabajo o comprobar los recursos consumidos por los trabajos.
...
:::image type="content" source="media/avs-backup/monitor-backup-jobs-in-mabs.png" alt-text="Supervisar el estado de los trabajos de copia de seguridad en Azure Backup Server":::
...
1. Antes de realizar la recuperación desde un punto de recuperación en línea, asegúrese de que la ubicación de almacenamiento provisional contiene suficiente espacio libre para alojar el tamaño sin comprimir completo de la máquina virtual que quiere recuperar. La ubicación de almacenamiento provisional se puede ver o cambiar mediante la ejecución del **Asistente para establecer la configuración de la suscripción**.
   ...
   :::image type="content" source="media/avs-backup/mabs-recovery-folder-settings.png" alt-text="Configuración de la carpeta de recuperación de Azure Backup Server":::
...
1. En la página **Seleccionar tipo de recuperación**, elija si quiere realizar la recuperación en la instancia original o en una nueva ubicación y, después, seleccione **Siguiente**.
...
1. En la pantalla **Especificar opciones de recuperación**, realice una de las siguientes opciones:
