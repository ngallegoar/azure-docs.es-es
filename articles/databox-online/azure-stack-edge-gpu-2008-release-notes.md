---
title: Notas de la versión de Azure Stack Edge (versión preliminar) | Microsoft Docs
...
ms.locfileid: "89322405"
---
# <a name="azure-stack-edge-with-gpu-preview-release-notes"></a>Notas de la versión de Azure Stack Edge con GPU (versión preliminar)

En las notas de la versión siguientes, se identifican los problemas críticos pendientes y los problemas resueltos de la versión preliminar de 2008 de los dispositivos Azure Stack Edge con GPU.

Las notas de la versión se actualizan continuamente y se van agregando a medida que se descubren problemas críticos que requieren una solución alternativa. Antes de implementar el dispositivo Azure Stack Edge, revise detenidamente la información que encontrará en las notas de la versión.

Esta versión de **Azure Stack Edge 2008** corresponde a la versión de software siguiente:

- **2.1.1328.1904**

## <a name="whats-new"></a>Novedades

Se han agregado las siguientes características en Azure Stack Edge 2008:

- **Clases de almacenamiento**: en la versión anterior, solo podía aprovisionar el almacenamiento de forma estática mediante recursos compartidos SMB o NFS para las aplicaciones con estado implementadas en el clúster de Kubernetes que se ejecuta en el dispositivo Azure Stack Edge. En esta versión, se han agregado clases de almacenamiento que permiten aprovisionar el almacenamiento de forma dinámica. Para obtener más información, consulte el tema sobre [administración del almacenamiento de Kubernetes en el dispositivo Azure Stack Edge con GPU](azure-stack-edge-gpu-kubernetes-storage.md#dynamicprovisioning). 
- **Panel de Kubernetes con servidor de métricas**: en esta versión, se agrega un panel de Kubernetes con un complemento de servidor de métricas. Puede usar el panel para obtener información general sobre las aplicaciones que se ejecutan en el dispositivo Azure Stack Edge, conocer el estado de los recursos del clúster de Kubernetes y consultar los errores que se han producido en el dispositivo. El servidor de métricas agrega el uso de la CPU y la memoria en los recursos de Kubernetes del dispositivo. Para obtener más información, consulte [Uso del panel de Kubernetes para supervisar el dispositivo Azure Stack Edge con GPU](azure-stack-edge-gpu-monitor-kubernetes-dashboard.md).
- **Azure Arc para Azure Stack Edge**: a partir de esta versión, puede implementar cargas de trabajo de aplicaciones en el dispositivo Azure Stack Edge a través de Azure Arc. Azure Arc es una herramienta de administración híbrida que permite implementar aplicaciones en los clústeres de Kubernetes. Para obtener más información, consulte el tema sobre [implementación de cargas de trabajo mediante Azure Arc en el dispositivo Azure Stack Edge](azure-stack-edge-gpu-deploy-arc-kubernetes-cluster.md).  

## <a name="known-issues"></a>Problemas conocidos 

En la tabla siguiente se proporciona un resumen de los problemas conocidos del dispositivo Azure Stack Edge.

| No. | Característica | Problema | Soluciones alternativas o comentarios |
| --- | --- | --- | --- |
| **1.** |Azure Stack Edge y Azure SQL | La creación de una base de datos SQL requiere acceso de administrador.   |Lleve a cabo los pasos siguientes en lugar de los pasos 1 y 2 de [https://docs.microsoft.com/azure/iot-edge/tutorial-store-data-sql-server#create-the-sql-database](https://docs.microsoft.com/azure/iot-edge/tutorial-store-data-sql-server#create-the-sql-database). <ul><li>En la interfaz de usuario local del dispositivo, habilite la interfaz de proceso. Seleccione **Proceso > Número de puerto > Habilitar para el proceso > Aplicar**.</li><li>Descargue `sqlcmd` en la máquina cliente desde https://docs.microsoft.com/sql/tools/sqlcmd-utility. </li><li>Conéctese a la dirección IP de la interfaz de proceso (el puerto que se habilitó), agregando ",1401" al final de la dirección.</li><li>El comando final tendrá el siguiente aspecto: sqlcmd -S {IP de interfaz},1401-U SA -P "Con1raseña!Segura".</li>Tras este cambio, los pasos 3 y 4 de la documentación actual deben ser idénticos. </li></ul> |
...
|**8.**|Clúster de Kubernetes|Kubernetes solo se admite en Azure Stack Edge con Helm v3 o una versión posterior. Para más información, consulte las [preguntas frecuentes: eliminación de Tiller](https://v3.helm.sh/docs/faq/).|
|**9.**|Azure Arc y Azure Stack Edge|No se admiten implementaciones de Azure Arc si hay un proxy web configurado en el dispositivo Azure Stack Edge.||
...
- [Preparación de la implementación de Azure Stack Edge con GPU](azure-stack-edge-gpu-deploy-prep.md)
