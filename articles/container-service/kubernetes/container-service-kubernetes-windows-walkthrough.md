...
Si usa Azure CloudShell, `kubectl` ya está instalado. Si desea instalarlo de forma local, puede usar el comando [az acs kubernetes install-cli](/cli/azure/acs/kubernetes).
...
Para configurar `kubectl` para conectarse al clúster de Kubernetes, ejecute el comando [az acs kubernetes get-credentials](/cli/azure/acs/kubernetes). En el ejemplo siguiente se descarga la configuración del clúster para el clúster de Kubernetes.
