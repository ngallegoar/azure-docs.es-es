...
Este artículo muestra distintas opciones para configurar una entidad de servicio para el clúster de Kubernetes. Por ejemplo, si instaló y configuró la [CLI de Azure](/cli/azure/install-az-cli2), puede ejecutar el comando [`az acs create`](/cli/azure/acs) para crear el clúster de Kubernetes y la entidad de servicio al mismo tiempo.
...
> [!IMPORTANT]
> Para crear una entidad de servicio, debe tener permisos suficientes para registrar una aplicación en su inquilino de Azure AD y asignar la aplicación a un rol en su suscripción. Para ver si tiene los permisos necesarios, [compruébelo en el portal](../../active-directory/develop/howto-create-service-principal-portal.md#permissions-required-for-registering-an-app).
>
...
Estos parámetros se pueden especificar al implementar el clúster de Kubernetes desde la [interfaz de la línea de comandos (CLI) de Azure](container-service-kubernetes-walkthrough.md), [Azure Portal](../dcos-swarm/container-service-deployment.md) u otros métodos.

>[!TIP]
>Al especificar el **identificador de cliente**, asegúrese de utilizar `appId`, no `ObjectId`, de la entidad de servicio.
>

En el ejemplo siguiente se muestra una forma de pasar los parámetros con la CLI de Azure. En este ejemplo se utiliza la [plantilla de inicio rápido de Kubernetes](https://github.com/Azure/azure-quickstart-templates/tree/master/101-acs-kubernetes).
...
Si ejecuta el comando [`az acs create`](/cli/azure/acs#az-acs-create) para crear el clúster de Kubernetes, tiene la opción para generar automáticamente una entidad de servicio.
...
Para comprobar la fecha de expiración de una entidad de servicio, ejecute el comando [az ad app show](/cli/azure/ad/app#az-ad-app-show) con el parámetro `--debug` y busque el valor `endDate` de `passwordCredentials` cerca de la parte inferior de la salida:
...
Si las credenciales de la entidad de servicio han expirado, use el comando [az ad sp reset-credentials](/cli/azure/ad/sp) para actualizarlas:
