...
Para seguir con el tutorial, necesita una clave de suscripción para Bing Spell Check API. Si no la tiene, necesitará:

* Una suscripción a Azure: [cree una cuenta gratuita](https://azure.microsoft.com/free/cognitive-services/).
* [Un recurso de Bing Spell Check](https://ms.portal.azure.com/#create/Microsoft.CognitiveServicesBingSpellCheck-v7)

## <a name="create-a-new-web-page"></a>Creación de una página web

Abra un editor de texto. Crear un nuevo archivo denominado, por ejemplo, spellcheck.html.

## <a name="add-html-header"></a>Adición de un encabezado HTML
...
La función getSubscriptionKey devuelve la clave de Bing Spell Check API. La recupera del almacenamiento local (es decir, una cookie) o, de ser necesario, la solicita al usuario.
...
La función auxiliar getSubscriptionKeyCookie pide al usuario el valor de la clave de Bing Spell Check API y devuelve el valor de la clave.
...
La función auxiliar getSubscriptionKeyLocalStorage primero intenta recuperar la clave de Bing Spell Check API al buscar la cookie adecuada. Si no se encuentra la cookie, pide al usuario el valor de la clave. A continuación, devuelve el valor de la clave.
...
La función renderSearchResults muestra los resultados especificados de Bing Spell Check API, mediante la impresión con sangría JSON.
...
## <a name="bingspellcheck-function"></a>Función bingSpellCheck

Se llama a la función bingSpellCheck cada vez que el usuario escribe texto en el campo del formulario HTML.
Toma dos parámetros: el contenido del campo de formulario HTML y la clave de Bing Spell Check API.

```html
function bingSpellCheck(query, key) {
```

Especifique el punto de conexión de Bing Spell Check API y declare un objeto XMLHttpRequest, que se usará para enviar solicitudes al punto de conexión.
...
Establezca el encabezado **Ocp-Apim-Subscription-Key** en el valor de la clave de Bing Spell Check API.
...
Envíe la solicitud. Cierre la función bingSpellCheck, la etiqueta **script** y la etiqueta **head**.
...
Cuando se carga la página web, asegúrese de que se tiene la clave de Bing Spell Check API, y pídala al usuario para, si es necesario.
...
Cree un formulario HTML con un campo de texto. Controle el evento `onsubmit` y llame a la función bingSpellCheck, para lo que usará el contenido del campo de texto y la clave de Bing Spell Check API.
...
Abra la página web en el explorador. En el símbolo del sistema, escriba la clave de suscripción de Bing Spell Check API. Escriba una consulta (por ejemplo, "Hollo, wlrd!") en el cuadro de texto **Spell Check** (Corrector ortográfico) y presione **ENTRAR**. A continuación, la página web muestra los resultados de la consulta.
