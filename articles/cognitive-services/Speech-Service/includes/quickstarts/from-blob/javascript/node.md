...
> [!div class="checklist"]
> * [Configuración del entorno de desarrollo y creación de un proyecto vacío](../../../../quickstarts/setup-platform.md?tabs=vs&pivots=programmming-language-javascript)
> * [Ha creado un recurso de Voz de Azure](../../../../overview.md#try-the-speech-service-for-free)
> * [Ha cargado de un archivo de origen en un blob de Azure](../../../../../../storage/blobs/storage-quickstart-blobs-portal.md)

## <a name="create-a-new-js-file"></a>Ha creado un nuevo archivo JS

El primer paso es asegurarse de que tiene el proyecto abierto en el editor que prefiera.

Llame al archivo index.js.
...
Al igual que las API REST, realizan solicitudes en formato JSON y también devuelven resultados en JSON.
Para facilitar la comprensión de solicitudes y respuestas, se declararán algunas clases para la serialización y deserialización del código JSON.
...
## <a name="create-an-initial-transcription-request"></a>Cree una solicitud de transcripción inicial.
A continuación, se generará la solicitud de transcripción.
...
## <a name="send-the-transcription-request"></a>Envíe la solicitud de transcripción.
...
A continuación, se llamará a un método `CheckTranscriptionStatus` para comprobar el estado y, finalmente, se imprimirán los resultados. A continuación, se va a implementar `CheckTranscriptionStatus`.
...
## <a name="check-the-requests-status"></a>Comprobación del estado de las solicitudes
...
`CheckTranscriptionStatus` toma la dirección URL del estado de la solicitud de transcripción y la sondea cada 5 segundos hasta que indica que la operación se ha realizado correctamente o con errores. A continuación, llama a `PrintResults` para imprimir los resultados de la transcripción. A continuación, se va a implementar `PrintResults`.
```csharp
...
**Inicie la aplicación** : ejecute el archivo index.js del nodo.
