...
description: Aprenda a usar los dos modos de detección de la API Anomaly Detector para detectar anomalías en los datos de serie temporal.
...
Use este inicio rápido para comenzar a usar Anomaly Detector API para detectar anomalías en los datos de serie temporal. Esta aplicación de C# envía dos solicitudes de API que contienen datos de serie temporal con formato JSON y obtiene las respuestas.
...
1. Cree una nueva función llamada `detectChangePoints()`. Construya la solicitud y envíela mediante una llamada a la función `Request()` con el punto de conexión, la dirección URL para la detección de anomalías por lotes, su clave de suscripción y los datos de la serie temporal.

2. Deserialice el objeto JSON y escríbalo en la consola.

3. Si la respuesta contiene un campo `code`, imprima el código de error y el mensaje de error.

4. En caso contrario, busque las posiciones de los puntos de cambio en el conjunto de datos. El campo `isChangePoint` de la respuesta contiene una matriz de valores booleanos, cada uno de los cuales indica si un punto de datos se identificó como un punto de cambio. Convierta esto en una matriz de cadenas con la función `ToObject<bool[]>()` del objeto de respuesta. Itere la matriz e imprima el índice de los valores `true`. Estos valores corresponden a los índices de los puntos de cambio de tendencia, si se encuentra alguno.
